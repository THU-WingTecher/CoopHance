...
class CarliniWagnerL2(Adversarial):
    """
    ...
    def build_attack(self, **kwargs):
        ...
        self.CW = CWL2(self.param, self.model, self.batch_size, self.confidence,
                    ...
                    self.clip_max, self.param.get_conf('classes_num'),
    ...
    def attack_on_data(self, x_val, y_val,process=(None,None), **kwargs):
        """
        ...
        num_selection  = m.ceil(len(x_val) * self.get_selection_rate())
        preds = self.model.predict(x_val)
        index_correct = np.where(preds == y_val)[0]
        x_val = x_val[index_correct[:num_selection]]
        y_val = y_val[index_correct[:num_selection]]
        ...
        if process[0] is not None:
            x_val = process[0](x_val)
        
        # feed_dict = {self.input_tensor: x_val,self.labels_tensor: to_categorical(y_val)}
        x_advs, l2_dis, loss, num_iter, score = self.attack_batch(x_val, y_val)
        # for name in self.feedable:
        #     feed_dict[new_kwargs[name]] = self.feedable[name]

        # x_advs = self.sess.run(self.x_adv, feed_dict)
        if process[1] is not None:
            x_advs = process[1](x_advs)
        
        # need = np.where(l2_dis > 1e-9)[0]
        # x_advs = x_advs[]
        # l2_dis = l2_dis[need]
        # loss = loss[need]
        # score = score[need]
        # preds = self.model.predict(x_advs)
        targets = y_val
        # num_iter = num_iter[need]
        num_images = len(x_advs)
        unsucc = np.sum(score>100)

        
       
        print("successful attack rate %.3f%%" %((num_images-unsucc)* 100 / num_images))
        print("average l2 distance is %.3f"%(np.mean(l2_dis)))
        print("average loss is %.3f"%(np.mean(loss)))
        print("average iteration is %.3f"%(np.mean(num_iter)))
        print("generated adversarial examples:", num_images)
        self.save_initialization()
        for idx in range(len(x_advs)):
            # if num_iter[idx] == 0:
            #     continue
            self.save_img(x_advs[idx], targets[idx], index_correct[idx])
        self.dump_img()
        return x_advs

    def attack_batch(self, x, label):
        return self.CW.attack(x, to_categorical(label, nb_classes = self.param.get_conf('classes_num')))
...
class CWL2(object):
    ...
    def attack(self, imgs, targets):
        ...
        for i in range(0, len(imgs), self.batch_size):
            ...
            adv_imgs, l2_dis[i:i+self.batch_size],loss[i:i+self.batch_size], \
                score[i:i+self.batch_size],num_iter[i:i+self.batch_size] = \
                    self.attack_batch(imgs[i:i + self.batch_size], targets[i:i + self.batch_size])
            r.extend(adv_imgs)
        r = np.array(r)
        
        return r, l2_dis, loss, num_iter, score
