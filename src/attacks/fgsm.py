...
class FGSM(Adversarial):
    def __init__(self, model, param,data_max=1,data_min=0, save_all=True):
        super(FGSM, self).__init__(model, param)
    ...
    def attack_batch(self, x, label):
        ...
        def update_all(x_test, y_test, tar, cur_iter):
            ...
            new_cur_idx_all = cur_idx_all[inds_unsuccess]
            ...
            return new_cur_idx_all, x_test[inds_unsuccess], y_test[inds_unsuccess], tar[inds_unsuccess]
        adv_x = copy.deepcopy(x)
        target = to_categorical(label, self.num_classes)
        y = copy.deepcopy(label)
        cur_idx_all, adv_x, y, target = update_all(adv_x, y, target, 0)

        adv_x = self.iteration([adv_x, target])[0]
        cur_idx_all, adv_x, y, target = update_all(adv_x, y, target , 1)
            # print(loop_i)
        if len(cur_idx_all) >0:
            min_iters[cur_idx_all] = 1
